from commons import *
import owiener
from Crypto.Util.number import long_to_bytes


def main():
  N = 
  e = 
  c = 

  # https://en.wikipedia.org/wiki/Wiener%27s_attack
  # wedlug ataku Weinera gdy e jest odpowiednio bliskie N, to da sie odzyskać łatwo klucz deszyfrujący d

  # https://sagi.io/2016/04/crypto-classics-wieners-rsa-attack/ - przykladowe rozwiazanie, problem z instalacja i uzyciem cf
  # pip install owiener

  d = owiener.attack(e, N)
  decrypted_message = decrypt(c, d, N)
  readable_message = long_to_bytes(decrypted_message)
  print(f'Solution is {readable_message}')


if __name__ == '__main__':
  main()